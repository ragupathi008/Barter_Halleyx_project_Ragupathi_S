...
function renderProducts(products) {
    ...
    //   imageUrl = product.imageUrl.includes('uploads/')
    //     ? `http://localhost:5000/${product.imageUrl.replace(/^\/?uploads\//, '')}`
    //     : `http://localhost:5000/uploads/${product.imageUrl.replace(/^\//, '')}`;
    // } else {
    //   imageUrl = 'http://localhost:5000/uploads/default.png';
    // }
        
        // Create image URL - critical fix
...
function getProductImageUrl(imagePath) {
  ...
  // Normalize all cases to: http://localhost:5000/filename.ext
  return `http://localhost:5000/${imagePath.replace(/^\/?uploads\//, '')}`;
}

// Usage in renderProducts():
...
function setupEventListeners() {
    ...
    document.getElementById('nextPage').addEventListener('click', () => {
        const totalPages = Math.ceil(
            filterProducts(allProducts, currentSearch).length / productsPerPage
        );
